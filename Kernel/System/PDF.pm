            ...
            if ( $Block{State} ) {
                ...
                while ($RowLoop) {
                    ...
                    if ( $Row <= $LastRow ) {
                        ...
                        {
                            for my $Column ( $Block{ReturnColumnStart} .. $Block{ReturnColumnStop} ) {
...
sub Image {
    ...
    # check needed stuff
    for my $Name (qw(File Width Height)) {
        if ( !defined( $Param{$Name} ) ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Name!"
...
sub _TableCalculate {
    ...
    for my $Row ( @{ $Param{CellData} } ) {
        ...
        for ( my $ColumnCounter = 0; $ColumnCounter < $ColumnMax; $ColumnCounter++ ) {
            ...
            # set default values
            for my $Property (qw(Type Font FontSize FontColor Align Lead BackgroundColor)) {
                if ( !defined( $Cell->{$Property} ) ) {
                    $Cell->{$Property} = $Param{$Property};
                }
            }
    ...
    for my $CurBlock ( @{$ColumnBlocks} ) {
        ...
        for my $Index ( $CurBlock->{ColumnStart} .. $CurBlock->{ColumnStop} ) {
            my $Column     = $Param{ColumnData}->[$Index];
            my $ExtraSpace = 0;
            if ( $Column->{Width} > 0 ) {
...
sub _TableBlockNextCalculate {
    ...
    # check needed stuff
    for my $Name (qw(CellData ColumnData)) {
        if ( !defined $Param{$Name} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Name!"
...
sub _TableRowCalculate {
    ...
    # check needed stuff
    for my $Name (qw(CellData RowData ColumnData Row)) {
        if ( !defined( $Param{$Name} ) ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Name!"
...
sub _TextCalculate {
    ...
    # check needed stuff
    for my $Name (qw(Text Type Width Height Font FontSize Lead)) {
        if ( !defined( $Param{$Name} ) ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Name!"
...
sub _StringWidth {
    ...
    # check needed stuff
    for my $Name (qw(Text Font FontSize)) {
        if ( !defined $Param{$Name} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Name!"
