...
sub Configure {
    ...
    my $Backends = join '|', sort $Self->_GetStorageBackends();

    $Self->Description('Migrate article files from one storage backend to another on the fly.');
    $Self->AddOption(
        Name        => 'target',
        Description => "Specify the target backend to migrate to ($Backends).",
        Required    => 1,
        HasValue    => 1,
        ValueRegex  => qr/^(?:$Backends)$/smx,
    );
    $Self->AddOption(
        Name        => 'source',
        Description => "Specify the source backend to migrate from ($Backends).",
        Required    => 0,
        HasValue    => 1,
        ValueRegex  => qr/^(?:$Backends)$/smx,
    );    
...
sub Run {
    ...
    my $Source = $TargetToSource{$Target} // $Self->GetOption('source');

    if ( !$Source ) {
        $Self->Print("<red>Source backend not specified.</red>\n");
...
sub _GetStorageBackends {
    ...
    my $BackendPath = 'Kernel/System/Ticket/Article/Backend/MIMEBase';
    my @Directories = map {
        $Kernel::OM->Get('Kernel::Config')->Get('Home') . $_ . $BackendPath
    } qw( / /Custom/ );
    ...
    for my $Dir (@Directories) {
        ...
        for my $File (@Files) {
            ...
            # Ignore the "Base.pm" file
            next FILE if $Basename eq 'Base';
