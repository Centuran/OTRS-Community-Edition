...
sub Run {
    ...
    # create tickets
    my @TicketIDs;
    for my $TicketNumber ( 1 .. $Self->GetOption('generate-tickets') ) {
        my $TicketUserID =
        ...
        if ($TicketID) {
            ...
            for my $ArticleNumber ( 1 .. $Self->GetOption('articles-per-ticket') // 10 ) {
...
sub CustomerCreate {
    ...
    for my $CustomerUserNumber ( 1 .. $Count ) {
        my $Name      = 'fill-up-user' . int( rand(100_000_000) );
