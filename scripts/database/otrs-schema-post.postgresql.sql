...
IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE LOWER(conname) = LOWER('FK_oauth2_token_config_id_id')
    ) THEN
    ALTER TABLE oauth2_token ADD CONSTRAINT FK_oauth2_token_config_id_id FOREIGN KEY (config_id) REFERENCES oauth2_token_config (id);
END IF;
END$$;
;
DO $$
